...
async function adicionarComentario(avaliacao, comentario) {
  const { data, error } = await supabase
    .from('comentarios')
    .insert([{ avaliacao, comentario }]);
  if (error) {
    return { success: false, error: error.message };
  }
  return { success: true };
...
export { supabase, adicionarContato, deletarContato, adicionarComentario, obterComentarios };
